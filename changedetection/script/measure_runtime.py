# Changed from infer_MambaBDA.py, only meant to measure the runtime.
# Call with:
# python script/measurements.py  --dataset 'xBD' --model_type MambaBDA_Base --test_dataset_path 'YOURPATH/xView2/' --test_data_list_path 'xView2/test_list.txt' \
# --cfg 'YOURPATH2/MambaCD/changedetection/configs/vssm1/vssm_base_224.yaml' --use_cuda --batch_size 1
#
# test_list.txt has to contain a list of images, following this format, which deviates from the original repo:
#
# test/images/midwest-flooding_00000333
# test/images/midwest-flooding_00000428
# test/images/hurricane-florence_00000082
import sys
    ...
    def infer(self):
        ...
        with torch.no_grad():
            for itera, data in enumerate(tqdm(val_data_loader)):
                ...
                time_measured = time.time() - start_time_cpu
                print(f'Inference time: {time_measured:.2f}')
                times.append(time_measured)

        print(f"Inference times - First time: {times[0]}, mean times: {torch.tensor(times).mean().item()}")
...
def main():
    ...
    # Setting --cuda False does not actually work. Bools need to be handled differently in argparse.
