...
class Trainer(object):
    ...
    def training(self):
        ...
        for _ in tqdm(range(elem_num)):
            ...
            # Having only a single building pixel in the batch means that a batch dimension gets lost in the loss output. Easiest to just ignore this edge case.
            ids, cnts = labels_clf.unique(return_counts=True)
            invalid_labels_clf =  len(ids) == 2 and ids[1] == 255 and cnts[0] <= 1

            valid_labels_clf = (labels_clf != 255).any() and not invalid_labels_clf
            ...
            try:
                ...
                # Shouldn't occur anymore after handling the edge case above, but if it does, we'll get some diagnostics
                print(f"IndexError raised with {labels_clf.shape=} {labels_clf.unique(return_counts=True)=}")
